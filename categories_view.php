	<div class="container">
        ...
        <? $character = range('A', 'Z');  
            ...
            foreach($character as $alphabet)
            {
            echo '<div class="row store '.strtolower($alphabet).'" style="text-transform:capitalize;"></div>'; 
            } 
        ?>
	</div>
<!--<p>Hello Store</p>-->
            ...
            $.ajax(
            {
                ...
                success: function(data)
                {
                    $.each(JSON.parse(data), function(index, item){
                        ...
                        var itemsj = items.join('-')
                        $('.'+dataid).append('<div><a href="/' + itemsj + '-coupons">' + item.name + '</a></div>');
                        //alert(item.name);
                    });
